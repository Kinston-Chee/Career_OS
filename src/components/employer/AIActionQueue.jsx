...
export default function AIActionQueue() {
  return (
    <section className="employer-glass-card flex h-full flex-col p-5">
      ...
      <p className="mt-1 text-xs text-slate-500">Recommended actions to move hiring forward</p>
        ...
        {actionQueue.map((action) => {
          ...
          return (
            <button
              key={action.id}
              type="button"
              className="flex w-full items-start gap-3 rounded-xl px-2 py-2.5 text-left transition hover:bg-white/60"
            >
