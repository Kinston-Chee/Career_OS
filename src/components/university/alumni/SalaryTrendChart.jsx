...
import { salaryTrend } from '../../../data/alumniSignalsData'
...
function indexToX(i) {
  ...
  return PAD_L + (i / (salaryTrend.years.length - 1)) * plotW
}

export default function SalaryTrendChart() {
  const points = salaryTrend.values.map((v, i) => [indexToX(i), valueToY(v)])
  const linePath = points.map(([x, y], i) => `${i === 0 ? 'M' : 'L'}${x},${y}`).join(' ')
  const areaPath = `${linePath} L${points[points.length - 1][0]},${H - PAD_B} L${points[0][0]},${H - PAD_B} Z`
        ...
        <div className="pointer-events-none absolute inset-0">
          {salaryTrend.annotations.map((ann, idx) => {
            ...
            return (
              <div
                key={ann.year}
                className="absolute -translate-x-1/2 rounded-lg border border-purple-200 bg-purple-50 px-2 py-1 text-center shadow-sm"
                style={{ left: `${leftPct}%`, top: idx % 2 === 0 ? '4%' : '14%', width: 130 }}
              >
